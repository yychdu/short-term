...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagD1)
    {
        info->event[info->num] = EVENT_D1;
        info->num++;
        dataPoints->valueD1 = gizStandardDecompressionValue(D1_BYTEOFFSET,D1_BITOFFSET,D1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagD2)
    {
        info->event[info->num] = EVENT_D2;
        info->num++;
        dataPoints->valueD2 = gizStandardDecompressionValue(D2_BYTEOFFSET,D2_BITOFFSET,D2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagD3)
    {
        info->event[info->num] = EVENT_D3;
        info->num++;
        dataPoints->valueD3 = gizStandardDecompressionValue(D3_BYTEOFFSET,D3_BITOFFSET,D3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagD4)
    {
        info->event[info->num] = EVENT_D4;
        info->num++;
        dataPoints->valueD4 = gizStandardDecompressionValue(D4_BYTEOFFSET,D4_BITOFFSET,D4_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueD1 != cur->valueD1)
    {
        GIZWITS_LOG("valueD1 Changed\n");
        ret = 1;
    }
    if(last->valueD2 != cur->valueD2)
    {
        GIZWITS_LOG("valueD2 Changed\n");
        ret = 1;
    }
    if(last->valueD3 != cur->valueD3)
    {
        GIZWITS_LOG("valueD3 Changed\n");
        ret = 1;
    }
    if(last->valueD4 != cur->valueD4)
    {
        GIZWITS_LOG("valueD4 Changed\n");
        ret = 1;
    }

    if(last->valuevol != cur->valuevol)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuevol Changed\n");
            ret = 1;
        }
    }
    if(last->valuefAX != cur->valuefAX)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuefAX Changed\n");
            ret = 1;
        }
    }
    if(last->valuefAY != cur->valuefAY)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuefAY Changed\n");
            ret = 1;
        }
    }
    if(last->valuefAZ != cur->valuefAZ)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuefAZ Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(D1_BYTEOFFSET,D1_BITOFFSET,D1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueD1);
    gizStandardCompressValue(D2_BYTEOFFSET,D2_BITOFFSET,D2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueD2);
    gizStandardCompressValue(D3_BYTEOFFSET,D3_BITOFFSET,D3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueD3);
    gizStandardCompressValue(D4_BYTEOFFSET,D4_BITOFFSET,D4_LEN,(uint8_t *)devStatusPtr,dataPoints->valueD4);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valuevol = gizY2XFloat(vol_RATIO,  vol_ADDITION, dataPoints->valuevol); 

    devStatusPtr->valuefAX = exchangeBytes(gizY2XFloat(fAX_RATIO,  fAX_ADDITION, dataPoints->valuefAX));     
    devStatusPtr->valuefAY = exchangeBytes(gizY2XFloat(fAY_RATIO,  fAY_ADDITION, dataPoints->valuefAY));     
    devStatusPtr->valuefAZ = exchangeBytes(gizY2XFloat(fAZ_RATIO,  fAZ_ADDITION, dataPoints->valuefAZ));     
